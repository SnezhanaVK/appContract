...
func DBaddComment(comment models.Stages) error {
	conn, err := db.ConnectDB()
	if err != nil {
		log.Fatal(err)
	}
...
func DBChengeStatusStage(id_stage int, id_status_stage int, comment string) error {
	...
	_, err = tx.Exec(`UPDATE stages 
    SET id_status_stage=$1
    WHERE id_stage=$2`,
    id_status_stage,
    id_stage)
    ...
    _, err = tx.Exec(`INSERT INTO history_states (id_stage, id_status_stage, data_create, comment)
        VALUES ($1, $2, NOW(), $3)`,
        id_stage,
        id_status_stage,
        comment)
